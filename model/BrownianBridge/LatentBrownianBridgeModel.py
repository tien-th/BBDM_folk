...
class LatentBrownianBridgeModel(BrownianBridgeModel):
    ...
    def get_parameters(self):
        if self.condition_key == 'SpatialRescaler':
            ...
            # params = itertools.chain(self.denoise_fn.parameters(), self.conf_net.parameters(), self.cond_stage_model.parameters())
        else:
            ...
            # params = itertools.chain(self.denoise_fn.parameters(), self.conf_net.parameters())
        return params
    ...
    def sample(self, x_cond, clip_denoised=False, sample_mid_step=False):
        x_cond_latent = self.encode(x_cond, cond=True)
        if sample_mid_step:
            temp, one_step_temp, conf = self.p_sample_loop(y=x_cond_latent,
                                                     context=self.get_cond_stage_context(x_cond),
            ...
            return out_samples, one_step_samples, conf
        else:
            temp, conf = self.p_sample_loop(y=x_cond_latent,
                                      context=self.get_cond_stage_context(x_cond),
                                      clip_denoised=clip_denoised,
                                      sample_mid_step=sample_mid_step)
            x_latent = temp
