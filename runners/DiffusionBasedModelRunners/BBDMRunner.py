...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        ...
        if write:
            ...
            if additional_info.__contains__('conf_loss'):
                self.writer.add_scalar(f'conf_loss/{stage}', additional_info['conf_loss'], step)
        return loss
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        #
        # sample = samples[-1]
        sample, conf1, conf2, conf3 = net.sample(x_cond, clip_denoised=self.config.testing.clip_denoised)
        sample.to('cpu')
        ...
        image_grid = get_image_grid(conf3.to('cpu'), grid_size, to_normal=False)
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, 'confidence_3.png'))
    ...
    def sample_to_eval(self, net, test_loader, sample_path):
        ...
        conf3_path = make_dir(os.path.join(sample_path, f'confidence_3'))
        ...
        for test_batch in pbar:
            ...
            for j in range(sample_num):
                sample, conf1, conf2, conf3 = net.sample(x_cond, clip_denoised=False)
                ...
                for i in range(batch_size):
                    ...
                    if j == 0:
                        ...
                        save_single_image(conf3[i], conf3_path, f'{x_name[i]}.npy', max_pixel=1,to_normal=False)
